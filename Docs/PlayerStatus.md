...
씨앗의 우클릭도 공격 이벤트로 전달되지 않습니다. 작물은 땅 종류와 분리된 `CropType`으로 저장되고, 현재 임시 작물인 `TestCrop`을 수량 제한 없이 심을 수 있습니다. 같은 밭에는 한 번만 심을 수 있으며 성장 단계에 따라 작은 갈색 구체 1~5개로 표시합니다. 30분 단위 파종 묶음과 1시간 단위 성장 규칙은 `CropGrowth.md`를 참고합니다.
